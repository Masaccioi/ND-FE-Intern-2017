// 框架版本
const APP_CORE = '1.0.0'

// 本地模拟
const SIMULATION = 0
// 开发
const DEVELOPMENT = 1
// 测试
const DEBUG = 2
// 生产
const PRODUCTION = 4
// 预生产
const PREPRODUCTION = 8
// 压测
const PRESSURE = 16
// 亚马逊
const AWS = 32
// 加利福利亚
const AWSCA = 64

const LOC_PROTOCOL = location.protocol + '//'
const LOC_HOST = location.host
// host === hostname:port
const LOC_HOSTNAME = location.hostname

/**
 * @constant {number} ENV
 */
const ENV = (() => {
  if (window.Bridge) {
    const appFactory = window.Bridge.require('sdp.appfactory')
    const config = appFactory.getServiceConfig({
      componentId: 'com.nd.sdp.component.model-source-app'
    })
    return +(config.MD_ENV || PREPRODUCTION)
  }
  switch (LOC_HOSTNAME) {
    case '127.0.0.1':
      return DEVELOPMENT
    case 'localhost':
      return DEVELOPMENT
    default:
      if (/\d+\.\d+\.\d+\.\d+/.test(LOC_HOSTNAME)) {
        return DEVELOPMENT
      }
      if (/\.dev\.web\.nd$/.test(LOC_HOSTNAME)) {
        return DEVELOPMENT
      }
      if (/\.debug\.web\.nd$/.test(LOC_HOSTNAME)) {
        return DEBUG
      }
      if (/\.qa\.web\.sdp\.101\.com$/.test(LOC_HOSTNAME)) {
        return PRESSURE
      }
      if (/\.beta\.web\.sdp\.101\.com$/.test(LOC_HOSTNAME)) {
        return PREPRODUCTION
      }
      if (/\.aws\.101\.com$/.test(LOC_HOSTNAME)) {
        return AWS
      }
      if (/\.awsca\.101\.com$/.test(LOC_HOSTNAME)) {
        return AWSCA
      }
      return PRODUCTION
  }
})()

const LOC_RES = {
  module: 'loc',
  protocol: LOC_PROTOCOL,
  host: LOC_HOST,
  ver: 'v0.1'
}

let UC_RES = {
  module: 'uc',
  protocol: 'https://',
  ver: 'v0.93'
}
let CS_RES = {
  module: 'cs',
  protocol: 'https://',
  ver: 'v0.1'
}
let PBL_RES = {
  module: 'pbl',
  protocol: 'https://',
  ver: 'v0.8'
}

let MD_RES = {
  module: 'model',
  protocol: 'https://',
  ver: 'v0.1'
}

let PY_RES = {
  module: 'privacy',
  protocol: 'https://',
  ver: 'v0.1'
}

switch (ENV) {
  case DEVELOPMENT:
    PY_RES.protocol = 'http://'
    PY_RES.host = 'privacy.dev.web.nd'
    break
  case DEBUG:
    PY_RES.protocol = 'http://'
    PY_RES.host = 'privacy.debug.web.nd'
    break
  case PREPRODUCTION:
    PY_RES.host = 'privacy.beta.101.com'
    break
  case PRESSURE:
    PY_RES.host = 'privacy.qa.101.com'
    break
  case PRODUCTION:
    PY_RES.host = 'privacy.sdp.101.com'
    break
  case AWS:
    PY_RES.host = 'privacy.aws.101.com'
    break
  case AWSCA:
    PY_RES.host = 'privacy.awsca.101.com'
    break
  default:
    PY_RES = LOC_RES
}

switch (ENV) {
  case DEVELOPMENT:
    MD_RES.protocol = 'http://'
    MD_RES.host = 'model.dev.web.nd'
    // MD_RES.host = '192.168.253.72'
    break
  case DEBUG:
    MD_RES.protocol = 'http://'
    MD_RES.host = 'model.debug.web.nd'
    break
  case PREPRODUCTION:
    MD_RES.host = 'model.beta.101.com'
    break
  case PRESSURE:
    MD_RES.host = 'model.qa.101.com'
    break
  case PRODUCTION:
    MD_RES.host = 'model.sdp.101.com'
    break
  case AWS:
    MD_RES.host = 'model.aws.101.com'
    break
  case AWSCA:
    MD_RES.host = 'model.awsca.101.com'
    break
  default:
    MD_RES = LOC_RES
}

switch (ENV) {
  case DEVELOPMENT:
  case DEBUG:
  case PREPRODUCTION:
  case PRESSURE:
    UC_RES.host = 'ucbetapi.101.com'
    CS_RES.host = 'betacs.101.com'
    PBL_RES.host = 'pbl4user.beta.101.com'
    break
  case PRODUCTION:
    UC_RES.host = 'aqapi.101.com'
    CS_RES.host = 'cs.101.com'
    PBL_RES.host = 'pbl4user.sdp.101.com'
    break
  case AWS:
    UC_RES.host = 'awsuc.101.com'
    CS_RES.host = 'awscs.101.com'
    PBL_RES.host = 'pbl4user.aws.101.com'
    break
  case AWSCA:
    UC_RES.host = 'uc-awsca.101.com'
    CS_RES.host = 'cs-awsca.101.com'
    PBL_RES.host = 'pbl4user.awsca.101.com'
    break
  default:
    UC_RES = LOC_RES
    CS_RES = LOC_RES
    PBL_RES = LOC_RES
}

/**
 * @constant {string} DATETIME_FORMAT 默认的时间日期格式
 */
const DATETIME_FORMAT = 'yyyy-MM-dd hh:mm:ss'

/**
 * @constant {string} DATE_FORMAT 默认的日期格式
 */
const DATE_FORMAT = 'yyyy-MM-dd'

/**
 * @constant {string} TIME_FORMAT 默认的时间格式
 */
const TIME_FORMAT = 'hh:mm:ss'

/**
 * @constant {string} TOAST_DURATION 默认提示信息显示毫秒数
 */
const TOAST_DURATION = 3000

/**
 * @constant {object}  I18N 多语言对应
 */
const I18N = {
  'zh-CN': '\u7b80\u4f53\u4e2d\u6587',
  'en-US': 'English',
  'id-ID': 'Bahasa Indonesia'
}

/**
 * @constant {string}  透明图片
 */
const DEFAULT_AVATAR = 'data:image/png;base64,'
/**
 * 单位
 * @type {Object}
 */
const UNITS = {
  'SECOND': 'SECOND',
  'MINUTE': 'MINUTE',
  'HOUR': 'HOUR',
  'KM': 'KM',
  'METER': 'METER',
  'GROUP': 'GROUP',
  'NUM': 'NUM',
  'CLUD': 'CLUD'
}

const config = {
  APP_CORE,
  SIMULATION,
  DEVELOPMENT,
  DEBUG,
  PRODUCTION,
  PREPRODUCTION,
  PRESSURE,
  AWS,
  ENV,
  LOC_RES,
  UC_RES,
  CS_RES,
  MD_RES,
  PY_RES,
  PBL_RES,
  DATETIME_FORMAT,
  DATE_FORMAT,
  TIME_FORMAT,
  TOAST_DURATION,
  I18N,
  DEFAULT_AVATAR,
  UNITS
}

Object.keys(config).map(key => {
  if (/^(?!LOC).+_RES$/.test(key)) {
    const { protocol, host, ver } = config[key]
    config[key].base = `${protocol}${host}/${ver}`
  }
})

export default config
